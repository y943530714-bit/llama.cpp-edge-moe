...
struct common_init_result::impl {
    ...
    std::unique_ptr<edge_moe_trace_cb_user_data> edge_moe_trace;
...
common_init_result::common_init_result(common_params & params, bool model_only) :
    ...
    if (!params.edge_moe_trace_path.empty()) {
        pimpl->edge_moe_trace = std::make_unique<edge_moe_trace_cb_user_data>(
            params.edge_moe_trace_path,
            params.edge_moe_trace_max_events);
        if (!pimpl->edge_moe_trace->ok()) {
            COM_ERR("failed to initialize edge MoE trace '%s'\n", params.edge_moe_trace_path.c_str());
            return;
        }
        if (params.cb_eval != nullptr) {
            COM_WRN("%s", "--edge-moe-trace replaces the existing eval callback\n");
        }
        cparams.cb_eval           = edge_moe_trace_cb_eval;
        cparams.cb_eval_user_data = pimpl->edge_moe_trace.get();
    }
...
struct llama_model_params common_model_params_to_llama(common_params & params) {
    ...
    mparams.memory_budget   = params.memory_budget;
