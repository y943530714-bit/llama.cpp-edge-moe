...
复用率来源（"每步专家组合都不同，为何能复用"）：缓存的复用单位是单个专家页而非每步组合。24,800 次 decode 访问中只有 2,562 个唯一 (层,专家) 对，即 **89.7% 的访问是对已见过专家的重用**；频次高度偏斜：按频次排序的 top-44/层覆盖 94.1% 访问、top-60/层覆盖 98.3%（router 是 token 隐状态的函数，同域 workload 的 token 分布相似 -> 路由集中；代码生成中 token 高度重复）。每步组合不同无妨——每步的 4 个选择大概率落在频次头部集合内，而这些页就在缓存里。单题 decode 唯一专家中位 41/层（~3.2GB，最大 84/层），在 3.4GB 全局 LRU 预算内。这是同域 workload 下的统计保证而非逐请求硬保证：换域会迁移热集，需 re-warm（prefill 即 warmer），这也是 admission 控制 + 层间预取的工程价值。

## 3.2 预热与 server 模式（2026-09-12 更新：根因找到并修复）

**根因**：server 加载尖峰 = repack 权重重打包（默认开启，为可 repack 张量填充 11.5GB 的 CPU_REPACK 私有缓冲，加载期 ~150-175MB/s 读满）+ 全文件 PrefetchVirtualMemory（22GB）。两者叠加把可用内存压穿 0.6GB。

**修复与效果**（`--no-repack --no-prefetch -fit off`，三者均已实现/启用）：

- server 加载 **13 秒完成**（此前无法完成），工作集 3.25GB，空闲内存无尖峰
- cli 加载 297s vs 洪泛版 795s（2.7x）；首批 decode 提速 2-3 倍（1.26/0.97 vs 2.47/3.05 s/tok）
- **持久 server + 2 题预热后连续 5 个请求的 DRAM 专家命中率：84.2% / 84.5% / 85.2% / 86.6%（同题与新题一致，不再崩塌）**，TPOT 1.53-2.26s
- 对照：cli 带洪泛预热后 59-63%，冷态 0-10%
...
## 3.1.4 存储与内存带宽实测（重要更正，2026-09-13）

硬件实测（bandwidth_bench.c，NO_BUFFERING 直读）：

| 介质 | 顺序读 | 随机 4KB QD1 | 平均延迟 |
|---|---|---|---|
| DRAM（DDR4-3200 双通道，理论 51.2 GB/s） | 实测读带宽 15-21 GB/s（triad 12.2），热节流明显 | - | - |
| **C: 东芝 KBG40ZNT256G NVMe（系统盘）** | **781 MB/s** | **29,108 IOPS / 113.7 MB/s** | **0.03 ms** |
| **D: 西数 WD20SPZX 2TB HDD（5400rpm SMR）——模型文件所在** | **97.9 MB/s** | **95 IOPS / 0.4 MB/s** | **10.5 ms** |

重大更正：模型文件放在 D: 的机械硬盘上（不是 SSD），本报告所有存储测量实为 HDD 数据。随机 4KB 延迟 HDD 与 NVMe 相差 306 倍，这解释了冷态 decode 的高代价（依赖 readahead 把专家切片的连续 0.5-0.8MB 读合并成顺序流才勉强维持 ~2s/tok）。

建议：把模型复制到 C:（NVMe，剩余空间足够容纳 22GB）。预期收益：冷态/跨域 miss 的代价下降 4-8 倍（顺序）到数百倍（随机），投机解码在 NVMe 上可能由负转正（验证批缺页 29k IOPS vs 95 IOPS）；现有命中优化仍然有效但关键性下降。
