...
## 2.1.1 迁移 NVMe 后复测（健康模型副本，SHA256 校验通过，2026-09-13）

首版 C: 副本被 robocopy 中断损坏（预分配文件大小不等于数据写完，SHA256 不匹配，曾导致草稿接受率假性崩到 4-6%、多请求提前 EOS），已重新复制并双端校验后复测：

- **opt1（skip k1=4/k2=16）on NVMe：TPOT 212-246ms（前两题热态）-> 725-1146ms（5 题混合缓存压力下），命中率 80-100%，server 加载 6s**。对比 HDD server 的 1533-2264ms：2-6 倍提升
- **skip + DFlash + relaxed verify on NVMe：TPOT 1244-1681ms，接受率恢复 50-79%**（确认此前 4-6% 是模型损坏伪影），命中率 63-100%。**NVMe 上投机仍为净负**（比纯 skip 慢约 2 倍）：I/O 已不是瓶颈（NVMe 781MB/s 使 miss 廉价），剩余成本是纯 CPU——每个验证周期消耗 DFlash 16 行块前向 + 目标模型 4-5 行验证，平均只接受 ~2.6 token，入不敷出。投机转正需要的不是更快的存储，而是更长的接受序列（更大 n_max/更好草稿）或可忽略的验证批成本

迁移 NVMe 最终收益：server 加载 13s -> 6s；TPOT（skip）1533-2264 -> 212-1146ms；多题混合下的波动大幅收窄
